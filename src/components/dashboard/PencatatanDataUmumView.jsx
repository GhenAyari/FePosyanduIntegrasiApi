...
export default function PencatatanDataUmumView() {
  ...
  const fetchRiwayat = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://127.0.0.1:8000/api/data-umum', {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await axios.post('http://127.0.0.1:8000/api/data-umum', formData, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      await axios.delete(`http://127.0.0.1:8000/api/data-umum/${id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });

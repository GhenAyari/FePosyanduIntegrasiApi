...
export default function KesehatanView() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [resAnak, resRemaja, resIbu, resLansia] = await Promise.all([
          axios.get('http://127.0.0.1:8000/api/warga/anak', { headers }),
          axios.get('http://127.0.0.1:8000/api/warga/remaja', { headers }),
          axios.get('http://127.0.0.1:8000/api/warga/ibu', { headers }),
          axios.get('http://127.0.0.1:8000/api/warga/lansia', { headers })
        ]);
  ...
  const openDraftModal = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const response = await axios.get(`http://127.0.0.1:8000/api/draf-pemeriksaan/${target}`, {

...
export default function AdminEksporView() {
  ...
  const fetchDataKesehatan = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://127.0.0.1:8000/api/admin/pemeriksaan/${SASARAN[tab]}?posyandu_id=${selectedPosyandu.id}`, {
  ...
  const handleHapusData = async (id) => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      await axios.delete(`http://127.0.0.1:8000/api/admin/pemeriksaan/${SASARAN[tab]}/${id}`, { headers: { 'Authorization': `Bearer ${token}` } });

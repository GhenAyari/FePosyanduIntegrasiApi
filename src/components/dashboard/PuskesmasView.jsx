...
export default function PuskesmasView() {
  ...
  const fetchDataKesehatan = async () => {
    ...
    try {
      ...
      // PERUBAHAN: Endpoint diubah ke rute khusus puskesmas
      const response = await axios.get(`http://127.0.0.1:8000/api/puskesmas/pemeriksaan/${SASARAN[tab]}?posyandu_id=${selectedPosyandu.id}`, {
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                {fotoArray.map((path, idx) => (
                  <div key={idx} style={{ flex: '1 1 calc(50% - 12px)', minWidth: '150px', border: '1px solid #ddd', borderRadius: '8px', overflow: 'hidden' }}>
                    <a href={`http://127.0.0.1:8000/storage/${path}`} target="_blank" rel="noreferrer" title="Klik untuk memperbesar">
                      <img src={`http://127.0.0.1:8000/storage/${path}`} alt={`Foto ${idx + 1}`} style={{ width: '100%', height: '150px', objectFit: 'cover', display: 'block' }} />
                    </a>
                  </div>
                ))}
              </div>
          ...
          <div className="card">
            <div className="section-head" style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
              ...
              <button className="btn btn-violet" onClick={() => window.print()}>
                <i className="bi bi-file-earmark-pdf-fill me-2"></i>Ekspor Semua ke PDF
              </button>
            </div>
                  ...
                  {isLoading ? (
                    ...
                    dataKesehatan.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td>
                          {/* PERUBAHAN: Tombol Hapus Dihilangkan 100% */}
                    ...
                    <tr><td colSpan="4" style={{ textAlign: 'center', padding: '30px', color: '#666' }}>Belum ada data rekam medis {SASARAN_NAMA[tab]} di Posyandu ini.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      {selectedPosyandu && (
        <div id="dokumen-cetak">
          ...
          <h4 style={{ textAlign: 'center', color: '#555', marginTop: 0, marginBottom: '24px' }}>
            Fasilitas: Posyandu {selectedPosyandu.nama} | Kategori: {SASARAN_NAMA[tab]}
          </h4>
          ...
          {dataCetak.length > 0 ? (
            ...
            <p style={{ color: '#666', fontStyle: 'italic', textAlign: 'center' }}>Tidak ada rekam medis yang dapat diekspor.</p>
          )}
        </div>
      )}

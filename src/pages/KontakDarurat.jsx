...
export default function KontakDarurat({ activePage, onNavigate, onDarurat }) {
  return (
    <div className="kontak-page">
      ...
      <main className="kontak-main">
        ...
        {/* Daftar Kontak 9 Posyandu */}
        <div className="kontak-header" style={{ marginTop: '40px' }}>
          <h2 className="kontak-main-title" style={{ fontSize: '24px' }}>Kontak 9 Posyandu</h2>
          <p className="kontak-main-subtitle">Daftar kontak ketua posyandu di Desa Loa Duri Ulu.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px', marginTop: '20px' }}>
          {POSYANDU_LIST.map((p, idx) => (
            <div key={idx} className="card" style={{ padding: '20px', borderRadius: '16px', border: '1px solid var(--line)', background: 'var(--surface)' }}>
              <div style={{ display: 'flex', gap: '16px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: `var(--${p.warna}-bg)`, color: `var(--${p.warna}-deep)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 4h3l2 5-2.5 1.5a11 11 0 0 0 5 5L14 13l5 2v3a2 2 0 0 1-2 2C9.5 20 4 14.5 4 7a2 2 0 0 1 1-3Z"/></svg>
                </div>
                <div>
                  <h4 style={{ fontSize: '15px', fontWeight: 700, margin: '0 0 4px 0' }}>Posyandu {p.nama}</h4>
                  <p style={{ fontSize: '12px', color: 'var(--ink-soft)', margin: '0 0 4px 0' }}>{p.alamat}</p>
                  <p style={{ fontSize: '14px', fontWeight: 800, color: 'var(--ink)' }}>{p.telp}</p>
                </div>
              </div>
            </div>
          ))}
